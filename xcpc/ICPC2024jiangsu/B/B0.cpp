...
void solve() {
    ...
    P ps[5], pt[5];
    for (int i = 0; i < 5; i++) {
        ps[i] = P(std::cos(s[i]), std::sin(s[i]));
        pt[i] = P(std::cos(t[i]), std::sin(t[i]));
    }
    ...
    for (int i = 0; i < 5; i++) {
        ans += (t[i] - s[i]) / 2;
        int j = (i + 2) % 5;
        ans += cross(pt[i], ps[j]) / 2;
        in[i] = lineIntersection(Line(pt[i], ps[j]), Line(pt[(i + 1) % 5], ps[(i + 3) % 5]));
        std::cout << in[i].x << " " << in[i].y << std::endl;
    }
    
    for (int i = 0; i < 5; i++) {
        ans -= cross(in[i], in[(i + 1) % 5]) / 2;
    }
